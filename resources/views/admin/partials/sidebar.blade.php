...
<div class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-slate-900 via-blue-900 to-cyan-900 transform transition-transform duration-300 ease-in-out lg:translate-x-0" 
    ...
    <nav class="mt-4 px-4 space-y-1 overflow-y-auto" style="max-height: calc(100vh - 180px);">
        ...
        <a href="{{ route('admin.invoices.index') }}" class="flex items-center px-4 py-2.5 text-gray-300 hover:bg-white hover:bg-opacity-10 rounded-lg transition {{ request()->routeIs('admin.invoices.index') || request()->routeIs('admin.invoices.show') || request()->routeIs('admin.invoices.create') || request()->routeIs('admin.invoices.edit') ? 'bg-white bg-opacity-20 text-white' : '' }}">
            <i class="fas fa-file-invoice w-5 mr-3"></i>
            <span>Invoices</span>
        </a>

        <a href="{{ route('admin.invoices.bulk.form') }}" class="flex items-center px-4 py-2.5 text-gray-300 hover:bg-white hover:bg-opacity-10 rounded-lg transition {{ request()->routeIs('admin.invoices.bulk.*') ? 'bg-white bg-opacity-20 text-white' : '' }}">
            <i class="fas fa-file-invoice-dollar w-5 mr-3"></i>
            <span>Generate Invoice</span>
        </a>

        <a href="{{ route('admin.invoices.debt-report') }}" class="flex items-center px-4 py-2.5 text-gray-300 hover:bg-white hover:bg-opacity-10 rounded-lg transition {{ request()->routeIs('admin.invoices.debt-report') || request()->routeIs('admin.invoices.customer-debt') ? 'bg-white bg-opacity-20 text-white' : '' }}">
            <i class="fas fa-money-bill-wave w-5 mr-3"></i>
            <span>Laporan Hutang</span>
            @php $customersWithDebt = \App\Models\Customer::where('unpaid_invoices_count', '>=', 3)->where('has_installment_plan', false)->count() ?? 0; @endphp
            @if($customersWithDebt > 0)
            <span class="ml-auto bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{{ $customersWithDebt }}</span>
            @endif
        </a>
